/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import * as nestAccessControl from "nest-access-control";
import * as defaultAuthGuard from "../../auth/defaultAuth.guard";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import { MapSkusToPackageService } from "../mapSkusToPackage.service";
import { AclValidateRequestInterceptor } from "../../interceptors/aclValidateRequest.interceptor";
import { AclFilterResponseInterceptor } from "../../interceptors/aclFilterResponse.interceptor";
import { MapSkusToPackageCreateInput } from "./MapSkusToPackageCreateInput";
import { MapSkusToPackageWhereInput } from "./MapSkusToPackageWhereInput";
import { MapSkusToPackageWhereUniqueInput } from "./MapSkusToPackageWhereUniqueInput";
import { MapSkusToPackageFindManyArgs } from "./MapSkusToPackageFindManyArgs";
import { MapSkusToPackageUpdateInput } from "./MapSkusToPackageUpdateInput";
import { MapSkusToPackage } from "./MapSkusToPackage";
@swagger.ApiBearerAuth()
@common.UseGuards(defaultAuthGuard.DefaultAuthGuard, nestAccessControl.ACGuard)
export class MapSkusToPackageControllerBase {
  constructor(
    protected readonly service: MapSkusToPackageService,
    protected readonly rolesBuilder: nestAccessControl.RolesBuilder
  ) {}

  @common.UseInterceptors(AclValidateRequestInterceptor)
  @nestAccessControl.UseRoles({
    resource: "MapSkusToPackage",
    action: "create",
    possession: "any",
  })
  @common.Post()
  @swagger.ApiCreatedResponse({ type: MapSkusToPackage })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async create(
    @common.Body() data: MapSkusToPackageCreateInput
  ): Promise<MapSkusToPackage> {
    return await this.service.create({
      data: {
        ...data,

        packageId: data.packageId
          ? {
              connect: data.packageId,
            }
          : undefined,

        skuId: data.skuId
          ? {
              connect: data.skuId,
            }
          : undefined,
      },
      select: {
        createdAt: true,
        id: true,

        packageId: {
          select: {
            id: true,
          },
        },

        skuId: {
          select: {
            id: true,
          },
        },

        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @nestAccessControl.UseRoles({
    resource: "MapSkusToPackage",
    action: "read",
    possession: "any",
  })
  @common.Get()
  @swagger.ApiOkResponse({ type: [MapSkusToPackage] })
  @swagger.ApiForbiddenResponse()
  @ApiNestedQuery(MapSkusToPackageFindManyArgs)
  async findMany(@common.Req() request: Request): Promise<MapSkusToPackage[]> {
    const args = plainToClass(MapSkusToPackageFindManyArgs, request.query);
    return this.service.findMany({
      ...args,
      select: {
        createdAt: true,
        id: true,

        packageId: {
          select: {
            id: true,
          },
        },

        skuId: {
          select: {
            id: true,
          },
        },

        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @nestAccessControl.UseRoles({
    resource: "MapSkusToPackage",
    action: "read",
    possession: "own",
  })
  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: MapSkusToPackage })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async findOne(
    @common.Param() params: MapSkusToPackageWhereUniqueInput
  ): Promise<MapSkusToPackage | null> {
    const result = await this.service.findOne({
      where: params,
      select: {
        createdAt: true,
        id: true,

        packageId: {
          select: {
            id: true,
          },
        },

        skuId: {
          select: {
            id: true,
          },
        },

        updatedAt: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.UseInterceptors(AclValidateRequestInterceptor)
  @nestAccessControl.UseRoles({
    resource: "MapSkusToPackage",
    action: "update",
    possession: "any",
  })
  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: MapSkusToPackage })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async update(
    @common.Param() params: MapSkusToPackageWhereUniqueInput,
    @common.Body() data: MapSkusToPackageUpdateInput
  ): Promise<MapSkusToPackage | null> {
    try {
      return await this.service.update({
        where: params,
        data: {
          ...data,

          packageId: data.packageId
            ? {
                connect: data.packageId,
              }
            : undefined,

          skuId: data.skuId
            ? {
                connect: data.skuId,
              }
            : undefined,
        },
        select: {
          createdAt: true,
          id: true,

          packageId: {
            select: {
              id: true,
            },
          },

          skuId: {
            select: {
              id: true,
            },
          },

          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @nestAccessControl.UseRoles({
    resource: "MapSkusToPackage",
    action: "delete",
    possession: "any",
  })
  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: MapSkusToPackage })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async delete(
    @common.Param() params: MapSkusToPackageWhereUniqueInput
  ): Promise<MapSkusToPackage | null> {
    try {
      return await this.service.delete({
        where: params,
        select: {
          createdAt: true,
          id: true,

          packageId: {
            select: {
              id: true,
            },
          },

          skuId: {
            select: {
              id: true,
            },
          },

          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
